...
action_view_task() {
  ...
  if [ -n "$feature_sess" ]; then
    ...
    if [ -d "$research_dir" ]; then
      ...
      [ -f "$research_dir/types-generated.txt" ] && feature_step=14 && feature_step_name="Frontend Types"
...
sync_interrogate_results() {
  ...
  if [ -f "$research_dir/types-generated.txt" ]; then
    feature_step=14; feature_step_name="Frontend Types"
  fi
  if [ -f "$research_dir/api-client-generated.txt" ]; then
    feature_step=15; feature_step_name="Frontend API"
  fi
  if [ -f "$research_dir/frontend-complete.txt" ]; then
    feature_step=16; feature_step_name="Frontend Pages"
  fi
  if [ -f "$research_dir/frontend-integrated.txt" ]; then
...
check_interrogate_status() {
  ...
  if [ -f "$research_dir/test-feedback-generated.txt" ]; then
    echo "complete"
