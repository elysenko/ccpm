...
-- =============================================================================
-- TABLE 16: TEST_RESULTS
-- Journey test execution results from /pm:test-journey
-- =============================================================================
CREATE TABLE test_results (
    id SERIAL PRIMARY KEY,
    session_name VARCHAR(255) NOT NULL,
    test_run_id VARCHAR(50) NOT NULL,   -- Groups results from same test execution (e.g., "run-20240115-143022")
    ...
    step_results JSONB DEFAULT '[]',    -- [{step_number, status, observation}]
    issues_found JSONB DEFAULT '[]',    -- [{step_number, description}]
...
CREATE INDEX idx_test_results_session ON test_results(session_name);
CREATE INDEX idx_test_results_run ON test_results(session_name, test_run_id);
CREATE INDEX idx_test_results_journey ON test_results(journey_id);

-- =============================================================================
-- TABLE 17: FEEDBACK
-- Persona feedback from test runs, generated by /pm:generate-feedback
-- =============================================================================
CREATE TABLE feedback (
    id SERIAL PRIMARY KEY,
    session_name VARCHAR(255) NOT NULL,
    test_run_id VARCHAR(50) NOT NULL,   -- Links to test_results from same run
    persona_id VARCHAR(50) NOT NULL,
    overall_rating INTEGER CHECK (overall_rating BETWEEN 1 AND 5),
    nps_score INTEGER CHECK (nps_score BETWEEN 0 AND 10),
    recommendation VARCHAR(20) CHECK (recommendation IN ('yes', 'no', 'maybe')),
    positives JSONB DEFAULT '[]',
    frustrations JSONB DEFAULT '[]',    -- [{theme, severity, journey_ref}]
    bugs JSONB DEFAULT '[]',            -- [{title, description, severity, steps_to_reproduce}]
    feature_requests JSONB DEFAULT '[]', -- [{title, description, priority}]
    test_context JSONB DEFAULT '{}',    -- {passed, failed, success_rate}
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX idx_feedback_session ON feedback(session_name);
CREATE INDEX idx_feedback_run ON feedback(session_name, test_run_id);
CREATE INDEX idx_feedback_persona ON feedback(persona_id);

-- =============================================================================
-- TABLE 18: ISSUES
-- Prioritized issues from feedback analysis, generated by /pm:analyze-feedback
-- =============================================================================
CREATE TABLE issues (
    id SERIAL PRIMARY KEY,
    session_name VARCHAR(255) NOT NULL,
    test_run_id VARCHAR(50) NOT NULL,   -- Links to feedback from same run
    issue_id VARCHAR(50),               -- I-001 format
    title VARCHAR(255) NOT NULL,
    description TEXT,
    category VARCHAR(50) CHECK (category IN ('bug', 'ux', 'performance', 'feature_request')),
    severity VARCHAR(20) CHECK (severity IN ('critical', 'high', 'medium', 'low')),
    mentions INTEGER DEFAULT 1,         -- How many personas reported this
    rice_score DECIMAL(10,2),           -- Prioritization score
    journey_refs JSONB DEFAULT '[]',    -- Related journey IDs
    persona_refs JSONB DEFAULT '[]',    -- Personas who reported
    status VARCHAR(20) DEFAULT 'open' CHECK (status IN ('open', 'triaged', 'prd_created', 'resolved')),
    created_at TIMESTAMPTZ DEFAULT NOW(),
...
CREATE INDEX idx_issues_session ON issues(session_name);
CREATE INDEX idx_issues_run ON issues(session_name, test_run_id);
...
-- SCHEMA VERSION
-- =============================================================================
COMMENT ON SCHEMA public IS 'Interview Schema v4.1.0 - Added test_results, feedback, issues tables for test feedback pipeline';
